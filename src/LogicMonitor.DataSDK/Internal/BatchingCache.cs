...
namespace LogicMonitor.DataSDK.Internal
{
    ...
    public class BatchingCache
    {
            ...
            if (body.Count != 0)
            {
                lock (_Lock)
                {
                    foreach (var item in body)
                    {
                        ...
                        try
                        {
                            response = BatchingCache.MakeRequest(path: path, method: "POST", body: bodystring, create: true);
        ...
        public static RestResponse MakeRequest(string body, string path = default, string method = default, bool create = true, bool asyncRequest = false)
        {
